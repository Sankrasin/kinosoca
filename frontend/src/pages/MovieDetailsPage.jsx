...
export default function MovieDetailsPage() {
  ...
  const checkWatchlistStatus = async () => {
    try {
      ...
      const inWatchlist = response.data.items.some((item) => item.movie_id === parseInt(movieId))
  ...
  return (
    <div className="space-y-12 pb-12 -mt-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 items-end">
            ...
            <div className="md:col-span-3 z-20 animate-fade-in">
              ...
              {movie.original_title && movie.original_title !== movie.title && (
                <p className="text-xl text-gray-400 font-medium tracking-wide mb-4">{movie.original_title}</p>
              )}
              
              <div className="flex items-center gap-3 flex-wrap mb-6">
                <span className="badge border-accent/30 text-white">{movie.release_year || 'N/A'}</span>
                <span className="badge border-accent/30 text-white">{movie.runtime ? `${movie.runtime} min` : 'N/A'}</span>
                <span className="badge border-accent/30 text-accent flex items-center gap-1 shadow-[0_0_10px_rgba(168,85,247,0.4)]">
                  <Star size={14} className="fill-accent" /> {movie.vote_average.toFixed(1)} <span className="text-gray-400 font-normal ml-1">({movie.vote_count} votes)</span>
                </span>
              </div>
              ...
              {movie.genres && movie.genres.length > 0 && (
                <div className="mt-8 flex flex-wrap gap-3">
                  {movie.genres.map((genre) => (
                    <span key={genre.id} className="badge bg-white/5 hover:bg-white/10 transition-colors cursor-default">
                      {genre.name}
                    </span>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {movie.cast && movie.cast.length > 0 && (
                <div className="glass p-6 rounded-3xl">
                  <h4 className="text-xl font-heading font-bold text-white mb-4">Top Cast</h4>
                    ...
                    {movie.cast.slice(0, 5).map((actor) => (
                      <div key={actor.id} className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-gray-400 font-bold overflow-hidden">
                          {actor.profile_path ? (
                            <img src={`https://image.tmdb.org/t/p/w200${actor.profile_path}`} className="w-full h-full object-cover" />
                          ) : actor.name.charAt(0)}
                        </div>
                        <div>
                          <p className="text-gray-200 font-semibold">{actor.name}</p>
                          {actor.character_name && <p className="text-sm text-gray-400">{actor.character_name}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {movie.crew && movie.crew.length > 0 && (
                <div className="glass p-6 rounded-3xl">
                  <h4 className="text-xl font-heading font-bold text-white mb-4">Director</h4>
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center text-gray-400 font-bold text-lg overflow-hidden">
                      {movie.crew[0]?.profile_path ? (
                        <img src={`https://image.tmdb.org/t/p/w200${movie.crew[0].profile_path}`} className="w-full h-full object-cover" />
                      ) : movie.crew[0]?.name.charAt(0)}
                    </div>
                    <p className="text-lg text-gray-200 font-semibold">{movie.crew[0]?.name || 'Unknown'}</p>
                  </div>
                  
                  {movie.keywords && movie.keywords.length > 0 && (
                    <div className="mt-8">
                      <h4 className="text-xl font-heading font-bold text-white mb-4">Tags</h4>
                      <div className="flex flex-wrap gap-2">
                        {movie.keywords.slice(0, 8).map((keyword) => (
                          <span key={keyword.id} className="text-xs font-semibold px-3 py-1 bg-white/5 text-gray-400 rounded-lg border border-white/5">
      ...
      {movie.watch_providers && movie.watch_providers.length > 0 && (
        <div>
          <h3 className="text-2xl font-bold text-gray-100 mb-4">Where to Watch</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {movie.watch_providers.map((provider) => (
              <ProviderBadge key={`${provider.provider_id}-${provider.access_type}`} provider={provider} />
            ))}
          </div>
        </div>
      )}
