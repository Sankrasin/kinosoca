...
export default function HomePage() {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const response = await searchApi.search({ query, page: 1, page_size: 20 })
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-12 lg:px-24">
          <div className="max-w-3xl animate-fade-in">
            <h1 className="text-5xl md:text-7xl font-heading font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-300 drop-shadow-2xl tracking-tighter leading-tight mb-6">
              Unlock Your Next <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-blue-500">Binge</span>
            </h1>
      ...
      {isSearching ? (
        ...
        <div className="animate-fade-in">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={clearSearch} className="btn-secondary text-sm px-4 py-2">
              Clear Results
            </button>
          </div>
